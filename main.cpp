#include "mbed.h"
#include <chrono> // Include chrono header for std::chrono durations
...
void displayNumber(int number, bool showDecimal, int decimalPos) {
    ...
    for (int i = 0; i < 4; i++) {
        ...
        // Use std::chrono::milliseconds as recommended by the compiler
...
int main() {
    ...
    s1.mode(PullUp);
    s3.mode(PullUp);
    // Use std::chrono::seconds as recommended by the compiler
    timerTicker.attach(&updateTime, std::chrono::seconds(1)); // 1-second updates
    ...
    while (1) {
        ...
        if (!s1) {
            ...
            minutes = 0;
            // Use std::chrono::milliseconds as recommended by the compiler
            ThisThread::sleep_for(std::chrono::milliseconds(200)); // Debounce
        }
